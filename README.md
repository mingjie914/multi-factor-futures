...
截至2026-09-14：intraday注册733个因子，权威有效库188条（含13个GP改写因子及1个SPEC迁移因子），
当前备选策略10组：默认 **多源稳衡(17)**＋9组观察备选；均由有效库成员构成。
实际默认方法为 `lw_abs`（IC均值与Ledoit-Wolf协方差求解后取绝对值）＋
Top10/Bottom10＋五大分组cap3＋分侧ERC；不与另一选项`diag_icir`混称。
实际持仓排名缓冲`production_portfolio.rank_exit_buffer`默认0（关闭），当前10组均未开启；
仅在原生执行路线显式使用，规则与验收见[交易成本与换手口径](docs/交易成本与换手口径.md)。
“默认”指生产方法回测/观察，不等于实盘批准；目标权重发布门保持关闭（NO_TARGETS）。

本轮98个新增因子统一正式检验已完成，20个已按授权追加有效库，未加入策略。
另原创12个补位因子已完成正式检验：1项按授权登记、11项未过FDR；未改变现有策略。
编号已统一为#1–733，与733个注册因子一一对应；早期34项原地补为#700–733，
注册名、公式、周期契约及位置不变。编号仅供检索，注册名仍是持久身份。
检验报告与性能记录见[产物索引](runs/README.md)。

当前10组的中文报告、净值与热力图已更新至2026-09-11；原截止2026-09-08的重叠段
净值逐点一致。成员、方向、方法及费用参数保持冻结，新增通用成本压力诊断。
