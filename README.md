...
截至2026-09-15：intraday注册733个因子，权威有效库188条（含13个GP改写因子及1个SPEC迁移因子），
当前备选策略10组：默认 **多源韧衡(18)**＋9组观察备选；均由有效库成员构成。
...
方法、费用及缓冲默认不变；新池30项成本/缓冲验证已完成，10项B0逐日精确复现。
中文净值、类型分布、缓冲热力图与逐组结论见策略候选库；不自动开启缓冲。
