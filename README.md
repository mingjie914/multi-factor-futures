...
截至2026-09-13：intraday注册623个因子，权威有效库167条（含13个GP改写因子及1个SPEC迁移因子），
策略目录13组：默认 **多源稳衡**（17因子）＋12组观察备选；新5组已登记，不替换默认。
...
#485/#491因果修复与固定13组验证已完成，旧8组账本精确不变；新5组的修复前排名不再沿用。
有效库保留各批次资格快照；#491仅更新best_period标注为H10，局部复核不是全库重新认证。当前进度、历史边界和本地报告入口见
...
组合入口无参数运行默认首选策略；修改IDE SETTINGS显式分支即可比较13组。
