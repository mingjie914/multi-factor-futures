...
74个是`6f+47f+21f`历史固定研究因子集合；588个是`intraday.py`现行日频输出发现池；4,049个是
...
数据运行源或品种增减必须修改同一个主框架契约并完成重验；不得为了某次研究另建一份
并行配置。只有用户明确指定的外部策略或版本分支才允许独立配置。

全量重检`intraday.py`时显式限定模块，避免把所有同频率注册因子混入同一假设族：

```powershell
& $PY main.py research --config config/default.yaml `
...
& $PY main.py backtest --config config/default.yaml `
...
| `config/default.yaml` | 唯一主框架配置（38 品种、10f观察基线、研究/回测/监控/主策略共用） |
