...
>
> **审计状态（2026-08-21）**：分钟合约根精确匹配、最终20%资产上限、完整60条历史
> IC、T+1收益账本、移仓与停牌冻结等修复已实施；清洗后Parquet截至2026-08-20的
> 严格门禁和全量测试均已闭环。全历史研究从599个历史注册类中确认588个可估计定义，
> 得到20个观察发现、13个相关簇；生产批准仍为0，目标权重发布批准门保持关闭。
>
...
通过检验 + 相关性门槛的因子只进入**观察候选库**；还需新OOS和人工批准才可进入生产。
...
数据源: 因子实现显式请求原始频率；1min/5min分别读取对应Parquet，30min由15min显式重采样
