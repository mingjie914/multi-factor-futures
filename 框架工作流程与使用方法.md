...
> 迁移期全量因子性能重放已完成。旧全历史运行得到的H20/C13仅作冻结对照；当前正式有效因子库从0开始重建，目标
> 权重发布批准门保持关闭。
>
...
- `factors/library/intraday.py` 当前注册606个日频输出类；完整运行时发现池数量应以本次registry实测为准
...
数量口径不得混用：当前正式有效因子库从0开始重建；历史75个是单切分观察结果，10f和
13个是旧组合/迁移对照；606个是`intraday.py`现行日频输出发现池；4,049个是2026-08-21导入全部
内置模块后的运行时注册表。统计有效、组合入选和生产批准是三个不同状态。
...
IDE直接运行项目根目录`run_factor_workflow.py`。日常默认
`WORKFLOW = FactorWorkflow.VALIDATE_FACTOR_BATCH`，并必须填写本次预先冻结的
`FACTOR_NAMES`；第一次建库或正式重建才显式选择`VALIDATE_ALL_INTRADAY`。正式准入使用
`date_range.start`至统一`research_cutoff`的冻结历史区间，并按日频政策读取预热。
...
- 层级 FDR q=0.10
- |IC| ≥ 0.01, |HAC t| ≥ 2.0
- 方向、稳健性、样本充分性及适用的成本覆盖后置门

Bonferroni、ICIR、命中率、单调性和换手均为诊断；相关性与OOS属于后续选择/组合验证。

### ③ 库内筛选与相关性门槛（后续组合阶段）

在`run_factor_workflow.py`显式选择
`FactorWorkflow.SELECT_EFFECTIVE_SUBSETS`并设置新的`SELECTION_RUN_ID`。该入口只读取当前
有效库，不接受候选池或旧75清单。

**规则**：进入同一后续组合的因子之间 **|corr| ≥ 0.5 → 只保留簇代表**（绝对值，负相关 -0.98 同样属冗余）。
0.5阈值、排名暴露相关、complete-linkage和冻结IS样本均由该标准入口固定并写入运行合同。
`main.py research --correlation`是探索/迁移工具，不代替本入口。
...
最小标识分两层：有效库只保存registry中的稳定`family`；本次选择产物保存运行内
`cluster_id`，并在`selection_summary.json`绑定相关度量、complete-linkage算法、0.5阈值和
冻结样本区间。`cluster_id`不得跨run比较，也不得回写有效库。

通过正式层级FDR及后置门槛的全部因子进入**有效因子库**。相关聚类只生成某个冻结
样本合同下的子集，不删除有效身份；方案冻结后再进行组合级Walk-forward、locked OOS和
真实前向观察。已经被历史研究查看过的日期不能追溯声明为未见样本。

周期字段不可混用：`input_bar_frequency`是实际读取的原始bar，必须强校验；
`signal_frequency`是因子暴露输出频率；`best_period`只是正式准入中最强的未来收益证据期。
策略YAML用`factor_library.enforce_effective_membership: true`校验成员身份，但组合的
`rebalance_freq`和`holding_period`独立设定，不从`best_period`推导。
...
主框架的收益模型层保留平行实现，选择只由确定策略的完整YAML收口：

```text
有效因子库 → 平行因子子集 → 周期/板块匹配
                              ├─ 默认组合方法：lw_abs → Top10/Bottom10 → cap3 → ERC → gross 2
                              └─ 显式模型挑战：sector_grouped_ridge / sector_grouped_lasso
                                         ↓
                         品种选择 → 风险模型 → 权重优化 → 回测/观察
```

在显式模型挑战分支中，Lasso沿用与Ridge相同的板块因子映射、训练窗口、前向收益目标和预测接口；特征只用
训练数据标准化，惩罚强度只用按日期排序的扩展窗选择。Lasso系数为零只是本次模型拟合
结果，不会删除因子、修改有效因子库或修改平行因子子集。启用时在策略YAML中显式设置
`alpha.type: sector_grouped_lasso`；`config/default.yaml`的模型层仍保持`sector_grouped_ridge`，
但组合比较入口的默认方法是上面的`production_portfolio`，不是模型层挑战。

确定策略若启用Lasso，应在该策略完整YAML中整体替换`alpha`块，不能把两套模型参数混写：

```yaml
alpha:
  type: sector_grouped_lasso
  params:
    fit_intercept: true
    min_samples_per_sector: 100
    fallback_to_global: false
    lasso_alphas: [0.000001, 0.00001, 0.0001, 0.001]
    lasso_cv_folds: 3
    max_iter: 5000
    tol: 0.00000001
```

下述10因子ICIR流程是历史固定观察策略的独立实现，不等同于主框架收益模型层：
...
数据源: 默认认证DuckDB运行源；因子显式请求原始频率，DuckDB提供1min/5min，30min由15min显式重采样；Parquet只作审计/回退
...
回测中的滚动IC监控也属于策略YAML的`backtest.ic_monitor`配置：默认保持开启及既有参数；
固定有效因子子集的候选比较可以显式设为`enabled: false`，表示不在回测运行期再次隐式改写
已选因子集合。这一开关不改变因子有效性检验、有效库或研究截止日。
...
| 研究基线 | `config/default.yaml` | 研究截止日、正式全历史准入、126/42观察、处理与检验政策 | 某一确定策略的独立选择 |
| 有效因子库 | `factor_library/library.json` | 有效成员、方向、最佳证据期与证据链接 | 因子子集、策略权重、生产批准 |
...
# 1. 正式因子检验：在 run_factor_workflow.py 设置
#    WORKFLOW=VALIDATE_FACTOR_BATCH、FACTOR_NAMES 与唯一 VALIDATION_RUN_ID。
& $PY -X utf8 -B run_factor_workflow.py

# 2. 人工核验已完成的run后：切换为 ADMIT_COMPLETED_RUN，设置
#    ADMISSION_RUN_DIR 与 ADMITTED_AT；不会重算因子。
& $PY -X utf8 -B run_factor_workflow.py

# 3. 有效库下游聚类与组合候选：切换为 SELECT_EFFECTIVE_SUBSETS，
#    并设置唯一 SELECTION_RUN_ID。
& $PY -X utf8 -B run_factor_workflow.py
...
#    默认VALIDATE_CONFIGURATIONS；确认后切换RUN_AND_COMPARE（默认生产方法基线）
#    只有显式比较模型/风险/优化器时才切换RUN_AND_COMPARE_CONFIGURED
#    早期6f/8f/13f快照重评切换RUN_AND_COMPARE_SNAPSHOT_AUDIT
#    六个历史快照（旧10/75条派生平衡与紧凑/6f/8f/13f）同图审计切换RUN_AND_COMPARE_ALL
