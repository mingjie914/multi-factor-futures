...
>
> **审计状态（2026-08-20）**：分钟合约根精确匹配、最终20%资产上限、完整60条历史
> IC、T+1收益账本、移仓与停牌冻结等修复已实施；清洗后Parquet截至2026-08-19的
> 严格门禁、全历史选约/价格/收益对账、全量测试和固定集合回测重建均已闭环。旧搜索
> 排序仍失效；新结果只重评固定集合，不证明当前有效池最优，目标权重发布批准门保持关闭。
>
...
- `factors/library/intraday.py` 当前注册 599 个类，其中 585 个为 `intraday_advanced`；完整运行时发现池为 4,060（2026-08-17 实测）
...
数据源: 统一频率路由；普通日内因子默认使用原生5min，声明 `force_1min` 的因子读取真1min
...
状态: 固定观察基线；固定集合正确性重建已完成但未证明最优，`config/target_publication.yaml` 关闭，关闭状态为 `NO_TARGETS`
```
