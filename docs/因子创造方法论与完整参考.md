...
> **当前状态（2026-08-21）**：`intraday.py` 已编号至 #565，另有 20 个 K/V 兼容名；
> 全历史审计提交599个历史类后，删除了11个无横截面辨识能力且无消费者的定义；当前
> 共注册588个类，其中574个为`intraday_advanced`。正文编号是历史批次编号，允许因
...
| 函数 | 作用 |
|------|------|
| `_get_minute_panel(data, dates, universe, freq)` | 按显式频率读取已配置Parquet；配置源失败时失败关闭 |
...
> 设计预期不等于当前交易方向；本轮全量结果中的统计方向只适用于观察候选，仍需冻结WF/OOS
...
- **编号**：注释块 `# N. 注册名 — 中文名`，N 必须连续、与注册一一对应（**当前主序列已编号到 #565，未来新因子从 #566 开始续编**）
- **注释格式**：
  ```
  # ════...
  # 566. intraday_xxx — 中文名
  # ════...
  ```
...
# 4) 正式入池相关性去重: 与既有因子 |corr|>=0.5 的需二选一并人工复核
# 5) 换手/成本/容量审核 → 组合层
```
...
当前编号已到 #565，另有 20 个保留原 K/V 系列编号。K/V 因子已并入 `intraday.py`：
