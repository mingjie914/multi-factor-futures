...
> **文档目的**：保存日内因子的创造方法、代码规范、首批 170 个历史编号及后续批次说明。
>
> **适用范围**：`factors/library/intraday.py`；正文早期因子表是历史快照，不是当前完整清单。
> **当前状态（2026-09-14）**：共733个注册因子，注释编号#1–733一一对应。
> 早期14个基础因子和20个补充/变体已原地补为#700–733；注册名、公式及周期契约不变。
> 不另设K/V或“登记序号”体系；物理顺序保留依赖关系，注册名是持久身份。
> 新因子须先核对源码及并行任务的编号占用；不从本页历史表末尾推断下一个编号。
> 源码实现和注册契约决定计算语义，docstring解释实际公式；是否有效另查权威有效库。
...
日内因子主要使用分钟 OHLCV；部分跨品种、板块或辅助因子直接使用日频面板，具体以
因子实现的数据入口为准。分钟输入通过统一入口获取：
...
运行时只通过统一数据接口读取已发布Parquet或认证DuckDB镜像；不再从MySQL、
DolphinDB、RDS或AkShare回退。数据或字段缺失时应失败关闭，不能以外部适配器或静默NaN代替。

> `30min` 请求由正式频率路由读取15分钟原始分区后显式重采样，OHLC分别使用
> first/max/min/last，volume使用sum；不得把15分钟数据直接改标签为30分钟。需要固定
> 交易时段边界的特殊因子仍可从真1分钟数据自行聚合：
> ```python
...
| 契约项 | 要求 | 说明 |
|--------|------|------|
| `frequency` | `"daily"` | 这是研究输出周期，不是原始输入频率；`core/factor_contract.py` 会强制校验 |
| `validation_horizons` | `(5, 10, 20)` | 注册时冻结、去重排序；持有期是 **bar 数语义**（daily 下 1 bar = 1 交易日） |
| `description` | 必填 | 用于注册表展示 |
| 时间可用性 | 逐因子核对 | 收盘后才能完整获得的T日信息只能预测T之后的收益；是否`shift(1)`由因子定义与信号可知时点决定，不能统一强制或统一删除 |
| `compute` 调用 | 接收本次完整 `dates` 与 `universe` | `FactorEngine.compute_factor` 调用 `factor.compute(data, dates, universe)`；**跨日 rolling/状态 dict 完全受支持** |
| 预热期 | 滚动窗口前的观测为 NaN | 正式研究按`validation_policy.warmup_days_by_frequency`预留，不使用固定倍数 |
...
class IntradayXxx20d(Factor):
    ...
    def compute(self, data, dates, universe):
        ...
        if "close" not in panel:
            raise ValueError("minute Parquet panel is missing required field: close")
...
| 函数 | 作用 |
|------|------|
| `_get_minute_panel(data, dates, universe, freq)` | 按显式频率读取主框架已配置数据源；失败时失败关闭 |
...
> 本节历史编号因子主要为`frequency="daily"`和`validation_horizons=(5,10,20)`；实际
> 输入频率、窗口与滞后以各类实现和注册契约为准，不可由本表概括反推。
> 设计预期不等于当前交易方向；长历史迁移结果中的统计方向只适用于观察候选，仍需滚动WF
> 和人工批准，历史方向与统计方向均不得自动形成新的生产结论。
...
- **编号**：注释块 `# N. 注册名 — 中文名`，新编号必须从当前最大编号连续递增并与注册一一对应；历史退役编号保留空号（**当前主序列最大编号为 #583，未来新因子从 #584 开始续编**）
- **注释格式**：
  ```
  # ════...
  # 584. intraday_xxx — 中文名
  # ════...
  ```
...
# 2) 编号递增 + 去重检查
#    新编号应连续递增；历史退役空号允许保留；@register_factor 名称必须无重复

# 3) 正式研究验证: 在 run_factor_workflow.py 填写完整 FACTOR_NAMES 批次后运行

# 4) 审阅正式run后显式入有效库；全部统计通过者均保留
# 5) 相关聚类/去冗、换手/容量审核 → 下游组合层
```
...
6. **跨日状态在单次 compute 内维护**：跨调用不保留状态，prev_xxx dict 必须每次 compute 从零开始
...
`intraday_advanced` 类别集中在 `intraday.py`。早期14个基础因子现为#700–713；
原20个补充/变体已按以下顺序原地补为#714–733，保留原注册名和依赖顺序：
...
**注意**：部分概念相近（如#714 `realized_kurtosis_20d`与#53 `intraday_realized_kurtosis_20d`），
名称不同但可能高相关。不得仅凭名字合并计算或继承准入资格；通过正式准入后，在组合层评估冗余。
编号统一不改变输入bar、输出频率、验证预测期、预声明方向和既有滞后。
...
> 本节是 2026-08 当时的编号快照（#438）；此后已继续到 #565。以下内容按历史原貌保留：
> - #393-432: 研报借鉴批次(A1-A15 微观量价 + B1-B8 跨期跨品种 + C 级；这里是因子批次编号，与策略分支编号无关);
...
> - 后续批次当时从 #439 续编。
> 当前编号完整性回归见 `tests/test_intraday_complement_factors.py::test_intraday_numbers_cover_each_registration_once`；期限结构面板历史修复见研报整理总结文档第十二章。
...
- `tests/test_intraday_rollover.py` 覆盖 trade_date 对齐、合成排除、换月 NaN、跨块边界、换月日历与面板一致、新因子无未来泄漏及多品种隔离；测试数以 pytest 收集结果为准。
