...
> **当前状态（2026-09-06）**：`intraday.py` 主序列已编号至 #583，另有 20 个 K/V 兼容名；
> 全历史审计提交599个历史类后，删除了11个无横截面辨识能力且无消费者的定义，此后新增
> 18个因子；正式扫描时为606个类。2026-09-06退役无效高耗时因子
> `intraday_scene_amp_flow_20d`后，当前共注册605个类，其中591个为
> `intraday_advanced`。正文编号允许因
> 退役出现空号；源码注册名与docstring是权威来源。
...
`intraday_advanced` 类别全部集中在 `intraday.py`。本节记录的是 #190 时的历史边界；
当前主序列最大编号为 #583，另有 20 个保留原 K/V 系列编号。K/V 因子已并入 `intraday.py`：
...
**注意**：这 20 个并入因子与 # 编号因子部分概念相近（如 `realized_kurtosis_20d` vs #53 `intraday_realized_kurtosis_20d`），名称不同但可能高相关，组合层做相关性去重时需一并纳入。K/V 系列保持独立，不并入 # 序列。
