...
> **当前状态（2026-08-17）**：`intraday.py` 已编号至 #565，另有 20 个 K/V 兼容名；
> 运行时共注册 599 个类，其中 585 个为 `intraday_advanced`。正文中的 170/190/210/438
> 均是对应章节形成时的历史批次边界，不是当前总数；源码注册名与 docstring 是权威来源。
...
| 防未来函数 | 输出前 `shift(1)` | 输出在T日的值只使用不晚于T-1的信息，并预测T+1收益；若内部用未来h日收益估计历史关系，必须至少滞后h+1日 |
...
> 设计预期不等于当前交易方向；生产和候选组合必须以 `docs/有效因子库.md` 的最近一次
> 重检 Rank IC 方向及各自冻结快照为准。例如 DTWS 当前为正向，而 drip_stone、
> peak_ridge_ratio、open_close_volume_ratio、price_delay、torrent_down 当前为负向。
