...
> 背景: `_get_minute_panel(freq="30min")` 实际返回 **15min bar**（`_FREQ_DIR_MAP["30min"]` 指向 15min 数据且无重采样），
> 因此 4 个 30min 因子统一改为**因子内自聚合**（1min → 30min），本文件为它们的权威用法/公式说明。
...
要点：
- `force_1min=True`：绕过 `_INTRADAY_FREQ=5min` 回退，强制 1min 原始数据
- `resample("30min")`：按整点/半点对齐（09:00/09:30/10:00...），夜盘独立成窗、不跨日合并
- 输出统一为**日频**（`frequency="daily"`，每交易日 1 个值），滚动平滑 + `shift(1)` 防未来
- 禁止直接 `_get_minute_panel(freq="30min")`（会静默使用 15min 数据，导致周期错配）
