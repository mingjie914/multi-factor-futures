...
| --- | --- | --- | --- |
| `mean_variance`（`research_only`） | 预期收益幅度已可靠校准，需要直接比较品种的对照实验 | 联合优化预测收益、协方差风险与换手成本 | 一个全局二次规划 |
...
- `mean_variance` 只读取 `risk_aversion` 与 `cost_penalty`；适合预期收益幅度已经校准、
  且希望在一个全局二次规划中同时权衡收益、协方差风险和换手成本的研究。
- `hierarchical_asset_risk_parity` 只读取同名配置块；适合期货板块结构优先、
  希望降低大协方差矩阵估计误差的默认配置。
- 两者不能串联。`meta_optimizer` 是完整子策略组合之后的可选上层配置，
  不属于上述二选一的品种优化步骤。
- 默认单次换手上限为 30%；交易成本仍按实际权重变化在回测中扣除。若旧仓已经违反
  品种、板块、净敞口或杠杆硬限制，硬风控优先，允许当期换手超过该软上限。
