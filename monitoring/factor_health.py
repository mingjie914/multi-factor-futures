...
- 回撤-反弹周期: 多空累计收益 20 日窗口峰谷检测, 创 20 日新高且维持 2 日确认反弹;
- 状态机: ACTIVE / WATCH / RETIRED(可再激活), 阈值与检验流程一致(IC=0.02, 连续 20 日).

纯增量模块, 只读复用外部数据(信号面板 + 日度收益), 不修改任何现有模块.
"""
...
def _daily_cross_section_ic(signal: pd.Series, returns: pd.Series) -> float:
    """单日截面 Pearson IC(信号 vs 当日收益); 有效品种 < 10 时返回 NaN."""
...
def daily_ic_series(signal: pd.DataFrame, returns: pd.DataFrame) -> pd.Series:
    """逐日截面 IC 序列(index=交易日, 对齐信号与收益)."""
    common = signal.index.intersection(returns.index)
    if len(common) == 0:
        return pd.Series(dtype=float)
    sig, ret = signal.loc[common], returns.loc[common]
    out = {}
    for dt in common:
...
def signal_portfolio_returns(signal: pd.DataFrame, returns: pd.DataFrame,
    ...
    """信号组合日度收益: 前 top_n 多头等权 - 后 top_n 空头等权.
    ...
    if multi_only:
        return long_ret
    return long_ret.sub(short_ret, fill_value=np.nan)
...
class FactorHealthMonitor:
    ...
    def update(self, signals: dict[str, pd.DataFrame], returns: pd.DataFrame) -> None:
        for name, direction in self.factors.items():
            ...
            self._daily_ic[name] = daily_ic_series(sig, returns)
            self._port_ret[name] = signal_portfolio_returns(
        ...
        dates = sorted(set().union(*[set(s.index) for s in signals.values()], set()))
        if dates:
            self._last_date = pd.Timestamp(max(dates))
    ...
    def _drawdown_cycle(self, port_ret: pd.Series) -> dict:
        ...
        return {
            ...
            "in_drawdown": bool(depth < -C.DD_THRESHOLD),
