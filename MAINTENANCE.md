...
`pytest.ini`把默认测试发现范围限定到`tests/`，避免递归收集本地研究的历史代码快照；
正式回归测试仍全部运行，不通过删除历史快照解决同名模块冲突。
...
- 派生bar聚合先按原始时间排序，再截断时间分箱；相关缓存使用独立`chronological_v1`标识，
  不能复用排序修补前的派生频率缓存，原生频率不变。2026-09-18研究已重算
  `intraday_entropy_burst_persistence_10d`并核对端点不变性，但原正式批次的FDR重准入尚未完成；
  工程验收不等于有效库重新认证，原研究合同和资格记录不回写。
...
既有`comparison_research/`和`exa-results/`整体作为本地冻结研究存档保留并由Git忽略，
不移动或重写其合同引用路径，不上传行情、账本、代码副本及缓存。方法论文字只在docs发布一份。
Git忽略不是备份；不可通过`git clean -fdx`清空这些证据或其他本地配置。
