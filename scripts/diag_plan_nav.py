"""diag_plan_nav — 方案对比: 因子集 x cap 组合, 输出净值图 (标注 OOS/实盘起点).

方案矩阵:
  S1: B3 6因子                       cap=0 / cap=3
  S2: B3 + seat_long_short_seat_ratio (7因子)  cap=0 / cap=3
  S3: 全部 12 个有效因子              cap=0 / cap=3
"""
import sys
...
NEW7 = dict(B3)
NEW7["intraday_seat_long_short_seat_ratio_20d"] = 1
ALL12 = {
    "intraday_jump_intensity_20d": -1,
    "intraday_price_peak_count_20d": 1,
    "intraday_realised_skewness_20d": 1,
...
def backtest(runner, factors, universe, start, end, cap=0):
...
def main():
    runner, _ = load_config("config/intraday_backtest.yaml"), None
    from core.config import load_config as lc
    cfg = lc("config/intraday_backtest.yaml")
    runner = PipelineRunner(config=cfg)
    universe = [s for s in ["A", "AG", "AL", "AU", "CU", "FU", "HC", "I", "IC", "IF", "IH",
                            "J", "JM", "M", "MA", "NI", "P", "RB", "RM", "RU", "SA", "SN",
                            "SR", "T", "TA", "TL", "TS", "Y", "ZN"]]
    plans = {
        "S1-6因子-cap0": (B3, 0),
        "S1-6因子-cap3": (B3, 3),
        "S2-7因子-cap0": (NEW7, 0),
        "S2-7因子-cap3": (NEW7, 3),
        "S3-12因子-cap0": (ALL12, 0),
        "S3-12因子-cap3": (ALL12, 3),
    }
    colors = {"S1-6因子-cap0": "#95a5a6", "S1-6因子-cap3": "#2ecc71",
              "S2-7因子-cap0": "#f39c12", "S2-7因子-cap3": "#e67e22",
              "S3-12因子-cap0": "#5dade2", "S3-12因子-cap3": "#2e86c1"}
    fig, ax = plt.subplots(figsize=(15, 8))
    print("方案 | 全段 | OOS(3-1~5-15) | 实盘(5-16~7-31)")
    print("-" * 90)
    results = {}
    for label, (F, cap) in plans.items():
        s = backtest(runner, F, universe, "2025-01-01", "2026-07-31", cap=cap)
        results[label] = s
        nav = (1 + s).cumprod()
        oos = s[(s.index >= pd.Timestamp("2026-03-01")) & (s.index <= pd.Timestamp("2026-05-15"))]
        live = s[s.index > pd.Timestamp("2026-05-15")]
        print(f"{label:<14} | {stat(s)} | {stat(oos)} | {stat(live)}")
        ax.plot(nav.index, nav.values, label=label, color=colors[label], linewidth=1.4)
    ...
    ax.set_title("因子集 x 板块配额 方案对比净值 (2025-01 ~ 2026-07)")
    ax.set_ylabel("净值")
    ax.legend(loc="upper left", fontsize=9)
    ax.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig("runs/plan_cap_nav.png", dpi=150)
    print("净值图: runs/plan_cap_nav.png")
